...
func walkRangeStmt(s *ast.RangeStmt) *ir.MetaForContainer {
	...
	if s.Tok.String() == ":=" {
		...
		SetVariable(valueIdent.Obj, registerLocalVariable(currentFunc, valueIdent.Name, G2T(elmType)))
	}
	if s.Key != nil {
...
func walkIndexExpr(e *ast.IndexExpr, ctx *ir.EvalContext) *ir.MetaIndexExpr {
	...
	meta.Type = G2T(GetElementTypeOfCollectionType2(collectionTyp))
	return meta
}
