...
func emitVariable(obj *ast.Object) {
	...
	default:
		panic(fmt.Sprintf("Unexpected type:%#v", getPrimType(typ)))
	}
}
...
func emitVariableAddr(obj *ast.Object) {
	...
	default:
		panic(fmt.Sprintf("Unexpected type:%s :%#v", getPrimType(decl.Type), decl.Type))
	}
}
				...
				case *ast.DeclStmt:
					...
					case *ast.GenDecl:
						...
						case *ast.ValueSpec:
							...
							var varSize int
							switch getPrimType(varSpec.Type)  {
							case T_STRING:
								varSize = 16
							case T_INT:
								varSize = 8
